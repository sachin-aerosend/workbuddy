...
# usage: emu-cat.ps1 where | tap | long | drag <dx> <dy> | menu <itemIndex> [-Items n]   (menu: long-press, then tap item N of n, 0-based)
param([string]$Action = 'where', [int]$A = 0, [int]$B = -600, [int]$Items = 9)
...
function Find-CatWindow { Overlays | Where-Object { ($_[2] - $_[0]) -lt 700 -and ($_[3] - $_[1]) -gt 60 -and ([math]::Abs(($_[2]-$_[0]) - ($_[3]-$_[1])) -lt 120) } | Select-Object -First 1 }
$f = Find-CatWindow
